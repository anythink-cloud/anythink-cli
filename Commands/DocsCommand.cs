...
public class DocsCommand : Command<DocsSettings>
{
    ...
    private static object BuildSpec() => new
    {
        ...
        api_urls = new
        {
            production     = ApiDefaults.ProdApi,
            dev            = ApiDefaults.DevApi,
            local          = ApiDefaults.LocalApi,
            billing        = ApiDefaults.BillingCloud,
            local_billing  = ApiDefaults.LocalBilling,
        },
        quick_start = new[]
        {
            "anythink signup",
        ...
        env_vars = new[]
        {
            new { name = "ANYTHINK_ORG_ID",          description = "Project org/tenant ID" },
            new { name = "ANYTHINK_API_KEY",          description = "API key (ak_...)" },
            new { name = "ANYTHINK_TOKEN",            description = "JWT access token" },
            new { name = "ANYTHINK_BASE_URL",         description = "Override base API URL for project commands" },
            new { name = "ANYTHINK_ENV",              description = "Environment preset: local | dev | prod" },
            new { name = "ANYTHINK_PLATFORM_ORG_ID",  description = "Central platform org ID (billing commands)" },
            new { name = "ANYTHINK_PLATFORM_TOKEN",   description = "JWT from platform login" },
            new { name = "ANYTHINK_PLATFORM_API_URL", description = "Override platform WebAPI URL" },
            new { name = "ANYTHINK_BILLING_URL",      description = "Override billing API URL" },
            new { name = "ANYTHINK_ACCOUNT_ID",       description = "Active billing account ID" },
        },
    ...
    private static string BuildMarkdown() => $$"""
        ...
        **Project-level** (override saved profile — useful for CI and scripted workflows):

        | Variable | Description |
        |----------|-------------|
        | `ANYTHINK_ORG_ID` | Project org/tenant ID |
        | `ANYTHINK_API_KEY` | API key (`ak_...`) |
        | `ANYTHINK_TOKEN` | JWT access token |
        | `ANYTHINK_BASE_URL` | Override base API URL |
        | `ANYTHINK_ENV` | Environment preset: `local` \| `dev` \| `prod` |
        ...
        **Platform** (billing / admin):
        ...
        | `ANYTHINK_PLATFORM_API_URL` | Override platform WebAPI URL |
        | `ANYTHINK_BILLING_URL` | Override billing API URL |
        ...
        | Environment | URL |
        |-------------|-----|
        | Production (default) | `{{ApiDefaults.ProdApi}}` |
        | Dev | `{{ApiDefaults.DevApi}}` |
        | Local | `{{ApiDefaults.LocalApi}}` |
        | Billing | `{{ApiDefaults.BillingCloud}}` |
        ...
        ```sh
        # Project commands — no interactive login needed
        export ANYTHINK_ORG_ID=<org-id>
        export ANYTHINK_API_KEY=ak_...
        anythink entities list
        anythink data list blog_posts --json

        # Platform operations (billing + project management)
        export ANYTHINK_PLATFORM_ORG_ID=<platform-org-id>
        export ANYTHINK_PLATFORM_TOKEN=<jwt>
        export ANYTHINK_ACCOUNT_ID=<account-id>
        anythink projects create "My App" --region lon1 --plan <plan-uuid>
        anythink projects use <project-id>
        ```
        """;
}
