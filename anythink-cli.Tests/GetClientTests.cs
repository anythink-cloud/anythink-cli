...
public class GetClientTests : IDisposable
{
    ...
    {
        var profile = new AnythinkCli.Config.Profile
        {
            OrgId          = "10",
    ...
    // ── Env-var fast path (no profile needed) ─────────────────────────────────

    [Fact]
    public void EnvVar_OrgId_And_ApiKey_Returns_Client_Without_Profile()
    {
        Environment.SetEnvironmentVariable("ANYTHINK_ORG_ID",  "99");
        Environment.SetEnvironmentVariable("ANYTHINK_API_KEY", "ak_envtest");
        try
        {
            var client = new TestCommand().CallGetClient();
            client.OrgId.Should().Be("99");
        }
        finally
        {
            Environment.SetEnvironmentVariable("ANYTHINK_ORG_ID",  null);
            Environment.SetEnvironmentVariable("ANYTHINK_API_KEY", null);
        }
    }
}
